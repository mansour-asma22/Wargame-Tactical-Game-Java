...
public class Unite {
    ...
    public boolean selected() {
        ...
        HashMap<Hexagone, String> action = interactionHex();

        Jeu.setDeplacementPossibleHash(deplacementPossible);
        Jeu.setinteractionHexHash(action);
    ...
    /*
    Cette fonction retourne tous les hexagones à portée d'une unité donnée et avec lesquels une interaction est possible
     */
    public MyHashMap<Hexagone, String> interactionHex() {
        Hexagone h = Jeu.getMap()[x][y]; // hexagone ou se situe l'unite
        MyHashMap<Hexagone, String> interactionHex = new MyHashMap<Hexagone, String>();
        ...
        while (!aExplorer.isEmpty()) {
            ...
            for (Hexagone v : hexagoneCourant.getListeVoisin()) { // on parcourt la liste de ses voisins
                if (!interactionHex.containsKey(v) && !aExplorer.containsKey(v)
                        && aExplorer.get(hexagoneCourant) + 1 <= porte) {
                    aExplorer.put(v, aExplorer.get(hexagoneCourant) + 1);
                }

            } // fin du parcours des voisins
            if (!interactionHex.containsKey(hexagoneCourant)) {
                for (Joueur j : Jeu.getListeJoueurs()) {
                    for (Unite u : j.getListeUnite()) {
                        if (u.getX() == hexagoneCourant.getX() && u.getY() == hexagoneCourant.getY()) {
                            if (u.getTeamUnite() == this.getTeamUnite()) {
                                if (this.getTypeUnite() == Jeu.orc && u != this && !acted) {
                                    interactionHex.put(hexagoneCourant, "allie");
                                }
                            } else if (!acted) {
                                interactionHex.put(hexagoneCourant, "ennemi");
                            }
                        }
                    }
                }
    ...
    /*
    Cette fonction calcule les dégâts infligés à une unité attaquée en fonction 
    des points d'attaque de l'unité attaquante, de sa défense et du bonus de défense de l'hexagone sur lequel elle se 
    trouve.
     */
